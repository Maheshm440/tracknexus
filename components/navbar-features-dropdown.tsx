...
import { Clock, TrendingUp, Eye, Users, ChevronDown, ArrowRight, CheckCircle } from "lucide-react"
...
const dropdownCategories: DropdownCategory[] = [
  {
    ...
    features: [
      {
        title: "Time Tracking",
        description: "Accurately track employee hours, automatically or manually",
      },
      {
      ...
      {
        title: "Task Tracking",
        description: "Track time per task to assess productivity and balance workload",
      },
      {
        title: "Time & Billing",
        description: "Turn tracked hours into billable entries for invoicing and payroll",
      },
      {
        title: "Invoice Generation",
        description: "Create detailed invoices based on tracked time",
      },
    ],
  },
  {
...
export function NavbarFeaturesDropdown() {
  const [activeTab, setActiveTab] = useState(dropdownCategories[0].id)
  const [isDropdownOpen, setIsDropdownOpen] = useState(false)

  const currentCategory = dropdownCategories.find((cat) => cat.id === activeTab) || dropdownCategories[0]
  ...
  return (
    ...
    >
      {/* Trigger Button */}
      <button className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-white hover:text-deloitte-green transition-colors">
        Features
      ...
      <AnimatePresence>
        {isDropdownOpen && (
          <motion.div
            ...
            className="absolute left-0 top-full mt-2 w-screen max-w-6xl z-50"
